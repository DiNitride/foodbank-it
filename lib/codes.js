...
export async function getCodes() {
    let codes = await db.query('SELECT * FROM `ReferralCode`')
    return codes
}
...
export async function insertCode(code, surname) {
    console.log(`Inserting code ${code} for surname ${surname}`)
    let { insertId } = await db.query('INSERT INTO `ReferralCode` VALUES (NULL, ?, ?, NULL, 0)', [code, surname])
