...
export async function getOneOrganisationById(id) {
  ...
  FROM `Organisation` \
  LEFT JOIN `User` ON Organisation.OrganisationManagerId = User.UserId \
...
export async function approveOrganisation(id) {
  ...
  await sendSms(org.OrganisationContactPhone, `Hi, your organisation application has been approved! A manager account has been created for ${org.OrganisationApplicantForename} with the following details.\n\nUsername: ${user.username}\nPassword: password\n\nPlease log in now and change your password!`)
...
export async function updateOrgManager(orgId, managerId) {
  await db.query('UPDATE Organisation SET OrganisationManagerId = ? WHERE OrganisationId = ?', [managerId, orgId])
  await db.end()
}
