...
export async function setOrderParcel(orderId, parcelId) {
  await db.query('UPDATE `Order` SET OrderParcel = ?, OrderStatus = \'ready\' WHERE  OrderId = ?', [parcelId, orderId])
  db.end()
}

export async function closeOrder(orderId) {
  await db.query('UPDATE `Order` SET OrderStatus = \'closed\', OrderClosed = NOW() WHERE  OrderId = ?', [parcelId, orderId])
  db.end()
}
