...
export async function getParcels() {
  let parcels = await db.query('SELECT ParcelId, ParcelComplete, ParcelDetails, IF(IFNULL(Order.OrderId, 0) AND Order.OrderStatus != \'open\', 1, 0) AS ParcelUsed FROM Parcel LEFT JOIN `Order` ON Order.OrderParcel = Parcel.ParcelId')
  await db.end()
  return parcels
}
